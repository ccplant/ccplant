...
| --- | --- | --- | --- |
| `claude-legacy` agent type | The frontend still offers `claude-legacy`; the backend only preserves `claude-acp`, `codex-acp`, `pi-ollama`, and `cursor` in `supportedAgentTypeOrDefault`, so `claude-legacy` becomes the empty/default path. | Remove it from `AgentApiType`, session/profile selectors, labels, guards, and related tests/docs. This is currently a misleading option rather than a working backend mode. | Check saved profiles/settings and session-start telemetry for the value, then remove with a migration to `claude-acp` or the configured default. |
...
| Root `kubernetesSession` / in-process manager mode | Old API deployment owns sessions; new mode uses `sessionManager.enabled=true` and its nested `sessionManager.kubernetesSession`. | Root Helm values/schema, `k8s-session-configmap`, deprecated env block in `deployment.yaml:591+`, server's legacy in-process manager construction, render tests and migration converter. | All releases have session manager enabled; `helm migrate verify` reports no legacy sessions; no rollback support to an old chart is required. |
...
- `auto`, `claude-acp`, `codex-acp`, `pi-ollama`, and `cursor` are active agent modes. Remove only `claude-legacy` unless product scope changes.
