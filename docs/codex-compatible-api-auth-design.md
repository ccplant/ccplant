...
認証方法の指定だけではプロファイルへ秘密値を保存しない。専用接続も指定する場合は次節の方式で暗号化保存する。接続のデフォルトモデルとプロファイル・リクエストのモデル上書き順は維持する。

### プロファイル専用の Base URL / API キー

互換 API の認証方法を選んだ場合、プロファイル編集画面から専用の接続を設定できる。
`config.codex_connection` / `config.claude_connection` に Base URL、API 認証方式、API キー、任意のデフォルトモデルを指定する。
保存済みキーは `has_api_key` で存在だけ返し、値は再表示しない。キーの省略はそのプロファイルの保存済みキーを保持する。
接続フィールド自体の省略も既存値を保持し、`null` を明示すると専用接続を削除して認証設定の継承に戻る。
プロファイルの認証方法を OAuth 等へ切り替えると専用接続は使用されない。

専用接続の Base URL とキーは一組として扱い、認証設定のキーを別の Base URL へ流用しない。
API キーは既存の暗号化サービスを使って保存し、暗号化が未設定の環境ではキーの保存を拒否する。
復号に失敗した場合は起動を失敗させ、別の認証方法へフォールバックしない。
...
個人設定をその上に重ねず、認証ファイルの配布元も指定チームに揃える。既存の `credential_source` よりこの明示指定を優先する。
プロファイルの専用 API 接続、プロファイル環境変数、開始リクエストの上書きは従来どおり優先する。
未指定または空文字列は従来の継承規則を維持する。

保存時にチームへの所属を検査し、起動時にもセッションのチーム権限を再検査する。
チームスコープのプロファイル・セッションは自チームだけを参照でき、別チームへの秘密値の共有を防ぐ。
指定チームの設定が未登録、または起動ユーザーがチームの所属を失った場合は起動を拒否し、個人の認証情報へフォールバックしない。
