...
ユーザーが実行中のセッションへ一時的な秘匿情報を登録した場合は、次のコマンドで取得してください：

```bash
ccplant client consume-secret
```

引数は不要です。セッション向けに登録された未取得の情報が古い順に1件だけ標準出力へ返ります。同じ情報は、ユーザーが明示的に再取得を許可した場合に限り、もう一度取得できます。取得した値を会話、ログ、またはコマンドライン引数へ表示しないでください。未取得の情報がない場合、コマンドは失敗します。
