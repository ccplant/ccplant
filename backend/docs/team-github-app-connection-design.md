...
CCPlant team uses a particular GitHub App installation when a session accesses
that connection. The binding owns the App ID, installation ID, and private-key
reference; the connection continues to own the GitHub host and organization
routing information.
...
- preserve existing personal OAuth-token and deployment-wide GitHub App behavior
  during migration;
- allow one GitHub App to have different installation IDs for different teams.
...
teams may use different GitHub Apps, or the same App with different installation
IDs.
...
`installation_id` is explicit in the first version. Automatic discovery is
ambiguous when an App has multiple installations and would require broader App
API access. It can be added later as a deliberate operation that previews and
persists the selected installation.
...
Validation includes positive numeric IDs, a parseable RSA or EC PEM private key,
...
installation endpoint. It returns App slug, account, repository selection, token
expiry, and permission names, but not the token or key.
...
The resolver takes the already authorized `team_id`, connection ID, and repository
full name. It loads the binding, resolves the private key, and calls GitHub's
installation-token endpoint. If repository restriction is enabled, request a
token restricted to the target repository and reject repositories not present in
the installation. Cache tokens in memory by `(binding_id, repository_set,
permissions)` until five minutes before expiry, with single-flight refresh. Do
not persist them.
...
- enter App ID and installation ID;
...
should distinguish invalid key, missing installation, insufficient repository
...
Do not use team ID, repository name, App ID, or installation ID as unbounded
metric labels.
...
2. Let administrators create bindings. Provide a dry-run report mapping each
   team's legacy installation ID to a proposed connection; do not copy a private
   key into every binding automatically.
...
- GitHub.com and GHES installation-token endpoint tests;
...
- fail-closed tests for disabled binding, expired/revoked key, installation
  mismatch, repository restriction, and GitHub outage;
