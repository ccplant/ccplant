...
func (r *Router) registerConditionalRoutes() error {
	...
	if r.server.settingsRepo != nil && r.handlers.settingsController != nil {
		...
		r.echo.GET("/external-session-managers/:id/operations/status", r.handlers.settingsController.GetExternalSessionManagerOperationalStatus, auth.RequirePermission(entities.PermissionSessionRead, r.server.container.AuthService))
		r.echo.GET("/external-session-managers/:id/operations/logs", r.handlers.settingsController.GetExternalSessionManagerLogs, auth.RequirePermission(entities.PermissionSessionRead, r.server.container.AuthService))
		r.echo.POST("/external-session-managers/:id/operations/restart", r.handlers.settingsController.RestartExternalSessionManager, auth.RequirePermission(entities.PermissionSessionCreate, r.server.container.AuthService))
		r.echo.POST("/external-session-managers/:id/operations/upgrade", r.handlers.settingsController.UpgradeExternalSessionManager, auth.RequirePermission(entities.PermissionSessionCreate, r.server.container.AuthService))
