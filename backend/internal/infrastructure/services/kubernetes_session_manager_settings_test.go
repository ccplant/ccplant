...
func TestBuildSessionSettings_SystemModelBaseURLDefaults(t *testing.T) {
	t.Setenv("OPENAI_BASE_URL", "https://system-openai.example/v1")
	t.Setenv("ANTHROPIC_BASE_URL", "https://system-anthropic.example")

	manager := &KubernetesSessionManager{config: &config.Config{}, k8sConfig: &config.KubernetesSessionConfig{}}
	defaultReq := &entities.RunServerRequest{}
	defaultSession := NewKubernetesSession("default-session", defaultReq,
		"test-deploy", "test-service", "test-pvc", "test-ns", 9000, nil, nil)
	defaults := manager.buildSessionSettings(context.Background(), defaultSession, defaultReq, nil)
	if got := defaults.Env["OPENAI_BASE_URL"]; got != "https://system-openai.example/v1" {
		t.Fatalf("OPENAI_BASE_URL = %q, want system default", got)
	}
	if got := defaults.Env["ANTHROPIC_BASE_URL"]; got != "https://system-anthropic.example" {
		t.Fatalf("ANTHROPIC_BASE_URL = %q, want system default", got)
	}

	req := &entities.RunServerRequest{
		ProfileEnvironment: map[string]string{
			"ANTHROPIC_BASE_URL": "https://profile-anthropic.example",
		},
		Environment: map[string]string{
			"OPENAI_BASE_URL": "https://request-openai.example/v1",
		},
	}
	session := NewKubernetesSession("test-session", req,
		"test-deploy", "test-service", "test-pvc", "test-ns", 9000, nil, nil)
	...
	if got := settings.Env["OPENAI_BASE_URL"]; got != "https://request-openai.example/v1" {
		t.Fatalf("OPENAI_BASE_URL = %q, want request override", got)
	}
	if got := settings.Env["ANTHROPIC_BASE_URL"]; got != "https://profile-anthropic.example" {
		t.Fatalf("ANTHROPIC_BASE_URL = %q, want profile override", got)
	}
}
