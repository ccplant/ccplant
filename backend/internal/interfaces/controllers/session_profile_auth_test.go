...
func TestProfileAuthMethodsValidationAndMerge(t *testing.T) {
	...
	cfg.SetParams(&entities.SessionParams{CodexDefaultModels: map[string]string{"auth_json": "gpt-account", "openai_compatible": "gateway-model"}, ClaudeDefaultModels: map[string]string{"oauth": "claude-oauth", "bedrock": "claude-bedrock"}})
	require.NoError(t, validateSessionProfileConfig(cfg))
	cfg.SetParams(&entities.SessionParams{CodexDefaultModels: map[string]string{"oauth": "wrong-agent"}})
	require.Error(t, validateSessionProfileConfig(cfg))
	cfg.SetParams(&entities.SessionParams{ClaudeDefaultModels: map[string]string{"oauth": "invalid\nmodel"}})
	require.Error(t, validateSessionProfileConfig(cfg))
