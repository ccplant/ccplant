...
import { GitHubOrganizationsInput } from '@/app/admin/github-connections/GitHubOrganizationsInput'
  ...
  return <>
    ...
    {loading ? <p className="py-8 text-sm text-gray-500">読み込み中...</p> : <div className="space-y-3">
      {connections.map((item) => <div key={item.id} className="rounded-lg border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
        ...
        <button onClick={() => { const callback = `${window.location.origin}/api/v1/auth/github-connections/callback`; void navigator.clipboard.writeText(callback); showToast('Callback URLをコピーしました', 'success') }} className="mt-4 inline-flex items-center gap-1 text-xs text-blue-600"><Copy className="h-3.5 w-3.5" />Callback URLをコピー</button>
      </div>)}
    ...
    {open && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><form onSubmit={submit} className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-xl bg-white p-6 shadow-xl dark:bg-gray-800">
      ...
      <div className="space-y-4">
        ...
        <GitHubOrganizationsInput value={form.organizations || []} onChange={organizations => setForm({ ...form, organizations })} />
