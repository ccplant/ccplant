...
const emptyForm: GitHubConnectionInput = {
  name: '', base_url: 'https://github.com', api_url: 'https://api.github.com', oauth_client_id: '', oauth_scope: 'read:user read:org project', enabled: true, show_on_login: true, organizations: [],
...
export default function GitHubConnectionsAdminPage() {
  ...
  const startEdit = (item: GitHubConnection) => {
    setEditing(item)
    setForm({ name: item.name, base_url: item.base_url, api_url: item.api_url || '', oauth_client_id: item.oauth_client_id || '', oauth_scope: item.oauth_scope || 'read:user read:org project', enabled: item.enabled, show_on_login: item.show_on_login !== false, organizations: item.organizations || [],
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      if (!editing) {
        ...
        await client.updateGitHubConnection(editing.id, { name: form.name, base_url: form.base_url, api_url: form.api_url, oauth_client_id: form.oauth_client_id, oauth_scope: form.oauth_scope, enabled: form.enabled, show_on_login: form.show_on_login, organizations: form.organizations })
  ...
  return <>
    ...
    {loading ? <p className="py-8 text-sm text-gray-500">読み込み中...</p> : <div className="space-y-3">
      {connections.map((item) => <div key={item.id} className="rounded-lg border border-gray-200 bg-white p-5 dark:border-gray-700 dark:bg-gray-800">
        ...
        <div className="mt-4 flex flex-wrap gap-2 text-xs"><span className={`rounded-full px-2 py-1 ${item.enabled ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>{item.enabled ? 'Enabled' : 'Disabled'}</span><span className="rounded-full bg-gray-100 px-2 py-1 text-gray-600">{item.show_on_login !== false ? 'ログインに表示' : 'ログインでは非表示'}</span><span className="rounded-full bg-gray-100 px-2 py-1 text-gray-600">{item.secret_source} / {item.secret_configured ? '設定済み' : '未設定'}</span><span className="rounded-full bg-gray-100 px-2 py-1 text-gray-600">{item.linked_identities || 0} identities</span>{item.organizations?.map(org => <span key={org} className="rounded-full bg-blue-50 px-2 py-1 text-blue-700">{org}</span>)}</div>
