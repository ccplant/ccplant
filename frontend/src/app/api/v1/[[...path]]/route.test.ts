...
describe('/api/v1 transport', () => {
  ...
  it('preserves the method, body, and critical headers for an SSE response', async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response('data: {"ok":true}\n\n', {
        status: 202,
        headers: { 'Content-Type': 'text/event-stream', ETag: '"stream"' },
      }),
    )
    const request = new NextRequest('https://ui.example.test/api/v1/s/share-token/events?cursor=1', {
      method: 'POST',
      headers: {
    ...
    expect(headers.get('x-forwarded-prefix')).toBe('/api/v1')
    expect(response.status).toBe(200)
  ...
  it('opens the downstream SSE response before the upstream fetch resolves', async () => {
    ...
    const request = new NextRequest('https://ui.example.test/api/v1/s/share-token/events', {
