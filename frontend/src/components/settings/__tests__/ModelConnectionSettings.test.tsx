...
describe('ModelConnectionSettings', () => {
  ...
  it('configures an agent-specific default model with built-in authentication', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '保存して使用' }))
    await waitFor(() => expect(onSave).toHaveBeenCalled())
    expect(onSave.mock.calls[0][0]).toMatchObject({ mode: 'auth_json', model: 'gpt-default', default_models: { auth_json: 'gpt-default' } })
  })
  it('keeps separate default models when switching authentication methods', async () => {
    const onSave = vi.fn().mockResolvedValue(undefined)
    render(<ModelConnectionSettings agent="codex" connection={{ mode: 'auth_json', model: 'gpt-account', default_models: { auth_json: 'gpt-account', openai_compatible: 'gateway-model' }, base_url: 'https://gateway.example/v1', authentication: 'none' }} onSave={onSave} />)
    expect(screen.getByLabelText('codex デフォルトモデル ID')).toHaveValue('gpt-account')
    fireEvent.change(screen.getByLabelText('codex 接続方式'), { target: { value: 'openai_compatible' } })
    expect(screen.getByLabelText('codex デフォルトモデル ID')).toHaveValue('gateway-model')
    fireEvent.click(screen.getByRole('button', { name: '保存して使用' }))
    await waitFor(() => expect(onSave).toHaveBeenCalled())
    expect(onSave.mock.calls[0][0]).toMatchObject({ mode: 'openai_compatible', model: 'gateway-model', default_models: { auth_json: 'gpt-account', openai_compatible: 'gateway-model' } })
  })
})
